...
function audioZoneForPage(page,communityTab){
  ...
  if(p==="juegos"||p==="ranking")return "arcade";
...
function AudioSettingsModal({show,onClose,mode,onModeChange,currentZone,musicOn,onToggleMusic,onNextTrack,uiTheme,onToggleTheme,trackName}){
  ...
  function ModeButton({id,icon,title,desc}){
    ...
    <div style={{display:"grid",gap:12}}>
      <Card style={{background:"linear-gradient(180deg,#FFF4D6,#E9D3A3)",border:`1.5px solid ${T.g300}`}}>
        ...
        <div style={{fontSize:".8rem",fontWeight:800,color:T.textSub,lineHeight:1.38,marginTop:4}}>
          Elige si quieres que toda la app mezcle la playlist completa o que cada zona use música más adecuada.
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))",gap:10,marginTop:12}}>
          <ModeButton id="random" icon="🔀" title="Aleatoria en toda la app" desc="Mezcla todas las canciones de la playlist sin importar la sección. Es el modo por defecto."/>
          <ModeButton id="ambient" icon="🌗" title="Ambientada por sección" desc="Inicio, tienda, perfil y arcade priorizan canciones con el ambiente de esa zona."/>
        </div>
      </Card>
      ...
      <Card style={{background:"linear-gradient(180deg,#101C15,#07100D)",border:`1.5px solid ${T.g300}`,color:T.white}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginTop:10}}>
          ...
          <div style={{borderRadius:14,padding:10,background:"rgba(255,244,214,.08)",border:"1px solid rgba(255,244,214,.14)"}}>
            ...
            <div style={{fontWeight:1000}}>{currentZone||"global"}</div>
          </div>
        </div>
      ...
      <div style={{fontSize:".76rem",fontWeight:800,color:T.textSub,lineHeight:1.35}}>
        Consejo: mantén pulsado el botón de sonido de la cabecera para volver a abrir este menú.
      </div>
    </div>
  </Modal>;
}
...
function AppCore(){
  ...
  useEffect(()=>{
    ...
    setBackgroundMusicMode(audioMode==="ambient"?"ambient":"random");
    setBackgroundZone(audioZoneForPage(page,communityTab));
  },[audioMode,page,communityTab]);
  ...
  function updateAudioMode(nextMode){
    ...
    setAudioMode(clean);
    setBackgroundMusicMode(clean);
    setBackgroundZone(audioZoneForPage(page,communityTab));
    if(isMusicPlaying()){
      nextMusicTrack(false);
      setTimeout(()=>showToast(clean==="ambient"?`Música ambientada: ${getBackgroundName()}`:`Playlist aleatoria: ${getBackgroundName()}`),60);
    }else{
      showToast(clean==="ambient"?"Música ambientada por sección activada":"Música aleatoria en toda la app activada");
    }
  }
