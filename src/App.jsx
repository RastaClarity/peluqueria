...
function Juegos({user,setUser,showToast,showPoints,setHelperPage,onOpenTops,onOpenTycoon,settings}){
  ...
  const catToneMap={
    todos:["#3EE6C7","#123E37"],
    destacados:["#F2C85B","#4A2F0D"],
    ranking:["#B878FF","#2A1747"],
    clasicos:["#35B8D0","#123E52"]
  };
  ...
  function GameCard({g,featured=false}){
    ...
    return <Card hover style={{opacity:played&&!isTycoon?0.94:1,background:`linear-gradient(155deg,rgba(7,11,9,.98) 0%,${deep}F2 58%,rgba(8,12,10,.96) 100%), radial-gradient(circle at 88% 16%,${accent}40,transparent 38%), radial-gradient(circle at 0% 100%,${accent}18,transparent 32%)`,border:`1px solid ${played&&!isTycoon?"rgba(255,244,214,.18)":accent+"88"}`,position:"relative",overflow:"hidden",color:"#FFF7DA",padding:0,boxShadow:`0 18px 42px rgba(0,0,0,.30), inset 0 1px 0 rgba(255,255,255,.08), 0 0 0 1px ${accent}12`}}>
      <div style={{position:"absolute",right:4,top:10,opacity:.42,transform:"rotate(-5deg)",pointerEvents:"none",zIndex:0}}><RastaCardIllustration type={artType} accent={accent} size={96}/></div>
      <div style={{position:"absolute",left:0,right:0,top:0,height:38,background:`linear-gradient(90deg,${accent}3D,rgba(255,255,255,.055),transparent)`,borderBottom:`1px solid ${accent}22`,zIndex:0}}/>
      <div style={{position:"absolute",left:0,right:0,bottom:0,height:6,background:`linear-gradient(90deg,${accent},rgba(255,255,255,.18),transparent)`}}/>
      <div style={{position:"absolute",top:9,left:12,zIndex:2,display:"inline-flex",alignItems:"center",gap:6,padding:"5px 9px",borderRadius:999,background:"rgba(255,244,214,.90)",border:`1px solid ${accent}88`,fontSize:".62rem",fontWeight:1000,color:"#211407",textTransform:"uppercase",letterSpacing:".045em",boxShadow:"0 5px 12px rgba(0,0,0,.16)",maxWidth:"calc(100% - 120px)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
        {status}
      </div>
      <div style={{position:"relative",zIndex:1,padding:"46px 12px 12px",display:"grid",gridTemplateColumns:"minmax(0,1fr) auto",gap:10,alignItems:"end",minHeight:132}}>
        <div style={{minWidth:0,maxWidth:"78%"}}>
          <div className="rc-card-title" style={{fontWeight:1000,fontSize:featured?"1.16rem":"1.04rem",marginTop:0,color:accent,textTransform:"uppercase",letterSpacing:".02em",lineHeight:1,textShadow:`0 0 12px ${accent}22`}}>{g.title}</div>
          <div style={{fontSize:".74rem",fontWeight:800,lineHeight:1.25,color:"rgba(255,247,218,.84)",marginTop:5,maxWidth:210,background:"rgba(255,255,255,.05)",border:`1px solid ${accent}18`,padding:"8px 9px",borderRadius:14}}>{g.desc}</div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:8}}>
  ...
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
        <button onClick={()=>onOpenTops?.("games")} style={{border:"2px solid rgba(53,184,208,.40)",borderRadius:20,padding:"13px 12px",background:"linear-gradient(135deg,rgba(53,184,208,.18),#E9F9FF 58%,#D8F3FB)",color:T.g800,fontWeight:950,cursor:"pointer",textAlign:"left",boxShadow:"0 10px 20px rgba(53,184,208,.16)"}}><div style={{fontSize:"1.6rem"}}>🏆</div><div>Ranking semanal</div><div style={{fontSize:".72rem",color:T.textSub,marginTop:3}}>Top por minijuego</div></button>
        <button onClick={()=>onOpenTops?.("general")} style={{border:"2px solid rgba(184,120,255,.38)",borderRadius:20,padding:"13px 12px",background:"linear-gradient(135deg,rgba(184,120,255,.20),#FFF1FD 58%,#F0D7FF)",color:T.g800,fontWeight:950,cursor:"pointer",textAlign:"left",boxShadow:"0 10px 20px rgba(184,120,255,.16)"}}><div style={{fontSize:"1.6rem"}}>👑</div><div>Ranking general</div><div style={{fontSize:".72rem",color:T.textSub,marginTop:3}}>Juego y comunidad</div></button>
      </div>

      <Card style={{marginBottom:14,background:"linear-gradient(135deg,#FFF8E6 0%,#F7E9C7 44%,#F3E0F7 100%)",border:`1.5px solid ${T.g200}`,boxShadow:"0 12px 26px rgba(20,8,4,.10)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,marginBottom:10}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(100px,1fr))",gap:8}}>
          {GAMES.filter(g=>g.id!=="tycoon").slice(0,7).map(g=>{const played=getPlayedToday(g.id,uid);const tone=toneMap[g.id]||["#3EE6C7","#123F32"];return <div key={g.id} style={{border:`1px solid ${played?tone[0]+"66":tone[0]+"88"}`,background:played?`linear-gradient(180deg,${tone[0]}22,rgba(255,255,255,.72))`:`linear-gradient(180deg,rgba(255,255,255,.86),${tone[0]}18)`,borderRadius:16,padding:9,textAlign:"center",boxShadow:`0 8px 18px ${tone[0]}15`}}><div style={{fontSize:"1.35rem"}}>{g.icon}</div><div style={{fontSize:".68rem",fontWeight:950,color:T.g800,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{g.title}</div><div style={{fontSize:".64rem",fontWeight:900,color:played?T.g600:tone[0]}}>{played?"cobrado":"pendiente"}</div></div>})}
        </div>
      </Card>

      <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:6,marginBottom:12}}>
        {categoryDefs.map(c=>{const tone=catToneMap[c.id]||[T.gold,T.g700];const active=category===c.id;return <button key={c.id} onClick={()=>{SFX.tab();setCategory(c.id);}} style={{minWidth:132,border:`2px solid ${active?tone[0]:tone[0]+"44"}`,borderRadius:18,padding:"10px 12px",background:active?`linear-gradient(135deg,${tone[0]}26,#FFF8E8 56%,${tone[0]}18)`:`linear-gradient(135deg,rgba(255,244,214,.72),rgba(255,255,255,.55))`,color:T.g800,fontWeight:950,cursor:"pointer",textAlign:"left",boxShadow:active?`0 8px 18px ${tone[0]}28`:"0 5px 12px rgba(20,8,4,.08)"}}><div>{c.icon} {c.label}</div><div style={{fontSize:".66rem",fontWeight:850,color:active?tone[1]:T.textSub,marginTop:2}}>{c.sub}</div></button>})}
      </div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",margin:"-2px 2px 10px",flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          <button onClick={()=>setActiveGame("runner")} style={{border:"1px solid rgba(53,184,208,.42)",background:"linear-gradient(135deg,rgba(53,184,208,.18),rgba(255,255,255,.76))",borderRadius:999,padding:"6px 9px",fontSize:".68rem",fontWeight:950,color:T.g800,cursor:"pointer"}}>✂️ Runner</button>
          <button onClick={()=>setActiveGame("gacha")} style={{border:"1px solid rgba(184,120,255,.42)",background:"linear-gradient(135deg,rgba(184,120,255,.18),rgba(255,255,255,.76))",borderRadius:999,padding:"6px 9px",fontSize:".68rem",fontWeight:950,color:T.g800,cursor:"pointer"}}>🎰 Gacha</button>
          <button onClick={()=>onOpenTycoon?.()} style={{border:"1px solid rgba(242,200,91,.46)",background:"linear-gradient(135deg,rgba(242,200,91,.24),rgba(255,255,255,.78))",borderRadius:999,padding:"6px 9px",fontSize:".68rem",fontWeight:950,color:T.g800,cursor:"pointer"}}>🏪 Tycoon</button>
        </div>
      </div>
...
function InternalHomeDashboard({user,onNavigate,unread={}}={}){
  ...
  return <div className="premium-home rc-visual-rework" style={{animation:"fadeSlide .34s ease",display:"grid",gap:14}}>
    ...
    <div className="rc-home-live-panels" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:14}}>
      <div style={{borderRadius:24,padding:16,background:"linear-gradient(140deg,rgba(22,14,8,.98) 0%,rgba(54,34,14,.96) 54%,rgba(25,17,10,.98) 100%), radial-gradient(circle at 100% 0%,rgba(242,200,91,.22),transparent 34%)",border:"1px solid rgba(255,210,98,.30)",color:"#FFF7DA",boxShadow:"0 18px 34px rgba(0,0,0,.22), 0 0 0 1px rgba(242,200,91,.08) inset"}}>
        ...
        {loading?<Spinner/>:<div style={{display:"grid",gap:8,marginTop:14}}>
          {pendingOrders[0]&&<button onClick={()=>onNavigate?.("gestion")} style={{textAlign:"left",border:"1px solid rgba(255,210,98,.34)",background:"linear-gradient(135deg,rgba(242,200,91,.16),rgba(255,255,255,.05))",borderRadius:16,padding:12,cursor:"pointer",color:"#FFF7DA",boxShadow:"0 10px 22px rgba(242,200,91,.10)"}}><div style={{fontWeight:1000}}>Pedido pendiente</div><div style={{fontSize:".78rem",fontWeight:820,opacity:.74}}>{pendingOrders[0].nombre||pendingOrders[0].cliente_nombre||"Nuevo pedido"} · {pendingOrders[0].estado||"pendiente"}</div></button>}
          {activeCoupons[0]&&<button onClick={()=>onNavigate?.("gestion")} style={{textAlign:"left",border:"1px solid rgba(95,240,200,.30)",background:"linear-gradient(135deg,rgba(95,240,200,.18),rgba(255,255,255,.05))",borderRadius:16,padding:12,cursor:"pointer",color:"#FFF7DA",boxShadow:"0 10px 22px rgba(95,240,200,.10)"}}><div style={{fontWeight:1000}}>Vale activo</div><div style={{fontSize:".78rem",fontWeight:820,opacity:.74}}>{activeCoupons[0].titulo||activeCoupons[0].codigo||"Cupón disponible"}</div></button>}
          {bestScore&&<button onClick={()=>onNavigate?.("juegos")} style={{textAlign:"left",border:"1px solid rgba(184,120,255,.28)",background:"linear-gradient(135deg,rgba(184,120,255,.18),rgba(255,255,255,.05))",borderRadius:16,padding:12,cursor:"pointer",color:"#FFF7DA",boxShadow:"0 10px 22px rgba(184,120,255,.10)"}}><div style={{fontWeight:1000}}>Mejor partida reciente</div><div style={{fontSize:".78rem",fontWeight:820,opacity:.74}}>{bestScore.game_id||bestScore.juego||"Arcade"} · {Number(bestScore.score||bestScore.points||0).toLocaleString("es-ES")} puntos de juego</div></button>}
      ...
      <div style={{borderRadius:24,padding:16,background:"linear-gradient(145deg,rgba(7,19,16,.98) 0%,rgba(15,54,44,.92) 56%,rgba(7,19,16,.98) 100%), radial-gradient(circle at 100% 0%,rgba(95,240,200,.20),transparent 36%)",border:"1px solid rgba(95,240,200,.30)",color:"#FFF7DA",boxShadow:"0 18px 34px rgba(0,0,0,.20), 0 0 0 1px rgba(95,240,200,.08) inset"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
          <div>
        ...
        <div style={{marginTop:15,display:"grid",gap:10}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
            <div style={{borderRadius:14,padding:"10px 8px",background:"linear-gradient(135deg,rgba(242,200,91,.18),rgba(255,255,255,.06))",border:"1px solid rgba(242,200,91,.24)",boxShadow:"0 8px 18px rgba(242,200,91,.10)"}}><div style={{fontSize:".7rem",fontWeight:900,opacity:.76}}>RP</div><div style={{fontWeight:1000,color:"#E0B84F"}}>{Number(user?.puntos||0).toLocaleString("es-ES")}</div></div>
            <div style={{borderRadius:14,padding:"10px 8px",background:"linear-gradient(135deg,rgba(95,240,200,.18),rgba(255,255,255,.06))",border:"1px solid rgba(95,240,200,.24)",boxShadow:"0 8px 18px rgba(95,240,200,.10)"}}><div style={{fontSize:".7rem",fontWeight:900,opacity:.76}}>RC</div><div style={{fontWeight:1000,color:"#5EF0C8"}}>{userRC(user).toLocaleString("es-ES")}</div></div>
            <div style={{borderRadius:14,padding:"10px 8px",background:"linear-gradient(135deg,rgba(184,120,255,.18),rgba(255,255,255,.06))",border:"1px solid rgba(184,120,255,.24)",boxShadow:"0 8px 18px rgba(184,120,255,.10)"}}><div style={{fontSize:".7rem",fontWeight:900,opacity:.76}}>XP</div><div style={{fontWeight:1000,color:"#FFF7DA"}}>{userXP(user).toLocaleString("es-ES")}</div></div>
          </div>
        </div>
      </div>

      <div style={{borderRadius:24,padding:16,background:"linear-gradient(145deg,rgba(20,11,8,.98) 0%,rgba(64,20,16,.92) 60%,rgba(20,11,8,.98) 100%), radial-gradient(circle at 100% 0%,rgba(217,74,53,.22),transparent 36%)",border:"1px solid rgba(217,74,53,.28)",color:"#FFF7DA",boxShadow:"0 18px 34px rgba(0,0,0,.20), 0 0 0 1px rgba(217,74,53,.08) inset"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
          <div>
        ...
        <div style={{marginTop:14,borderRadius:18,padding:14,background:"linear-gradient(135deg,rgba(217,74,53,.14),rgba(255,255,255,.03))",border:"1px solid rgba(217,74,53,.20)",boxShadow:"0 10px 22px rgba(217,74,53,.10)"}}>
      ...
      <div style={{borderRadius:24,padding:16,background:"linear-gradient(145deg,rgba(11,15,22,.98) 0%,rgba(17,37,54,.92) 58%,rgba(11,15,22,.98) 100%), radial-gradient(circle at 100% 0%,rgba(53,184,208,.24),transparent 38%)",border:"1px solid rgba(53,184,208,.26)",color:"#FFF7DA",boxShadow:"0 18px 34px rgba(0,0,0,.20), 0 0 0 1px rgba(53,184,208,.08) inset"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
          <div>
        ...
        <button onClick={()=>onNavigate?.("comunidad")} style={{marginTop:14,border:"1px solid rgba(53,184,208,.34)",background:"linear-gradient(135deg,rgba(53,184,208,.18),rgba(255,255,255,.04))",borderRadius:14,padding:"10px 14px",fontWeight:1000,color:"#79E9FF",cursor:"pointer",boxShadow:"0 10px 22px rgba(53,184,208,.10)"}}>Ver comunidad</button>
      </div>
    </div>
  </div>;
}
...
function GlobalUIPolishPatch(){
  return <style>{`
    ...
    /* 2.9.9q · Pulido móvil general */
    *{box-sizing:border-box;}
    img,svg,canvas,video{max-width:100%;}
    button,input,textarea,select{font:inherit;}
    button{touch-action:manipulation;}
    .page-content-pro .card,
    .page-content-pro .studio-panel{
    ...
    @media(max-width:760px){
      ...
      .modal-panel-pro .card,
      .modal-panel-pro .studio-panel{
        border-radius:18px!important;
      }
      .premium-home-grid-main,
      .shop-command-layout{
    ...
    @media(max-width:560px){
      .page-content-pro{
        padding:10px 7px calc(118px + env(safe-area-inset-bottom,0px))!important;
      ...
      .page-content-pro .card .card,
      .page-content-pro .studio-panel .card{
        border-radius:15px!important;
      }
      .page-content-pro [style*="grid-template-columns:repeat(4,1fr)"],
      .page-content-pro [style*="grid-template-columns:repeat(4, 1fr)"]{
        grid-template-columns:repeat(2,minmax(0,1fr))!important;
      }
      .page-content-pro [style*="grid-template-columns:repeat(3,1fr)"],
      .page-content-pro [style*="grid-template-columns:repeat(3, 1fr)"]{
        grid-template-columns:repeat(3,minmax(0,1fr))!important;
      }
      .page-content-pro [style*="grid-template-columns:1fr 1fr 1fr"]{
        grid-template-columns:repeat(2,minmax(0,1fr))!important;
      }
      .page-content-pro [style*="grid-template-columns:repeat(auto-fit,minmax(250px,1fr))"],
      .page-content-pro [style*="grid-template-columns:repeat(auto-fit,minmax(260px,1fr))"],
      .page-content-pro [style*="grid-template-columns:repeat(auto-fit,minmax(280px,1fr))"]{
        grid-template-columns:1fr!important;
      }
      .page-content-pro [style*="grid-template-columns:92px 1fr"]{
        grid-template-columns:82px 1fr!important;
      }
      .shop-reward-card{
        border-radius:18px!important;
      }
      .shop-reward-card [style*="min-height:124"]{
      ...
      .rc-home-live-panels > div{
        min-height:auto!important;
      }
      .rc-arcade-games-grid{
        grid-template-columns:1fr!important;
      }
      .rc-arcade-games-grid .card{
        min-height:auto!important;
      }
      .bottom-nav-pro{
      ...
      .rasta-helper-fixed-safe [style*="width: min(318px"]{
    ...
    /* 2.9.9s · Cream Glass + Night Neon Refresh */
    :root{
      --rc-cream-page:#f4ead8;
      --rc-cream-card:#fff3d5;
      --rc-cream-card-2:#ead4a5;
      --rc-ink-day:#17100a;
      --rc-muted-day:#654828;
      --rc-blue-glass:#dff7ff;
      --rc-blue-glass-2:#bcefff;
      --rc-blue-line:rgba(34,174,220,.42);
      --rc-night-ink:#08080d;
      --rc-night-navy:#0b1020;
      --rc-night-purple:#17112c;
      --rc-night-coffee:#21120b;
      --rc-night-text:#fff5d4;
      --rc-night-soft:#cabd96;
    }

    [data-rc-theme="day"].app-shell,
    [data-rc-theme="day"].rc-standalone-shell{
      color:var(--rc-ink-day)!important;
      background:
        radial-gradient(circle at 12% 0%,rgba(94,210,255,.34),transparent 30%),
        radial-gradient(circle at 90% 8%,rgba(255,202,92,.32),transparent 30%),
        radial-gradient(circle at 82% 88%,rgba(196,142,255,.22),transparent 32%),
        linear-gradient(180deg,#f9efd9 0%,#efe2c5 48%,#e7d7b6 100%)!important;
      box-shadow:0 0 0 1px rgba(72,50,24,.10),0 0 56px rgba(65,160,200,.22)!important;
    }

    [data-rc-theme="day"] .page-content-pro{
      color:var(--rc-ink-day)!important;
      background:
        linear-gradient(180deg,rgba(255,250,232,.74),rgba(238,222,188,.70)),
        radial-gradient(circle at 18% 0%,rgba(110,216,255,.22),transparent 30%),
        radial-gradient(circle at 90% 14%,rgba(255,207,91,.18),transparent 28%),
        repeating-linear-gradient(135deg,rgba(80,55,20,.035) 0 1px,transparent 1px 38px)!important;
      border:1px solid rgba(120,84,32,.18)!important;
      box-shadow:inset 0 1px 0 rgba(255,255,255,.70),0 18px 52px rgba(90,58,22,.12)!important;
      backdrop-filter:blur(18px) saturate(1.05);
    }

    [data-rc-theme="day"] .app-header-pro,
    [data-rc-theme="day"] .rc-balanced-topbar{
      background:
        linear-gradient(135deg,rgba(7,20,34,.88),rgba(19,72,98,.78) 55%,rgba(36,27,52,.72)),
        radial-gradient(circle at 18% 0%,rgba(96,225,255,.28),transparent 36%)!important;
      border:1px solid rgba(190,239,255,.32)!important;
      color:#fff7df!important;
      box-shadow:0 16px 38px rgba(26,68,90,.22),inset 0 1px 0 rgba(255,255,255,.18)!important;
      backdrop-filter:blur(18px) saturate(1.15);
    }

    [data-rc-theme="day"] .brand-home-button,
    [data-rc-theme="day"] .header-action-pro,
    [data-rc-theme="day"] .wallet-button-pro,
    [data-rc-theme="day"] .cart-button-pro,
    [data-rc-theme="day"] .theme-toggle-pro{
      color:#fff7df!important;
      background:rgba(215,246,255,.14)!important;
      border:1px solid rgba(220,248,255,.25)!important;
      box-shadow:inset 0 1px 0 rgba(255,255,255,.20),0 8px 18px rgba(10,22,36,.16)!important;
    }

    [data-rc-theme="day"] .studio-panel,
    [data-rc-theme="day"] .card,
    [data-rc-theme="day"] .modal-panel-pro{
      color:var(--rc-ink-day)!important;
      background:
        linear-gradient(155deg,rgba(255,248,224,.90),rgba(238,216,168,.80)),
        radial-gradient(circle at 16% 0%,rgba(168,231,255,.18),transparent 34%)!important;
      border:1px solid rgba(134,93,36,.22)!important;
      box-shadow:0 16px 34px rgba(99,68,24,.13),inset 0 1px 0 rgba(255,255,255,.62)!important;
    ...
    [data-rc-theme="day"] .card b,
    [data-rc-theme="day"] .card strong,
    [data-rc-theme="day"] .studio-panel b,
    [data-rc-theme="day"] .studio-panel strong{
      color:#130c07;
    }

    [data-rc-theme="day"] input,
    [data-rc-theme="day"] textarea,
    [data-rc-theme="day"] select{
      background:rgba(255,251,236,.88)!important;
      color:#15100b!important;
      border:1.5px solid rgba(55,155,198,.32)!important;
      box-shadow:inset 0 1px 0 rgba(255,255,255,.72),0 4px 12px rgba(80,50,20,.08)!important;
    }

    [data-rc-theme="day"] input::placeholder,
    [data-rc-theme="day"] textarea::placeholder{
      color:rgba(50,34,20,.58)!important;
    }

    [data-rc-theme="day"] .bottom-nav-pro{
      background:
        linear-gradient(135deg,rgba(219,247,255,.78),rgba(180,230,255,.55) 55%,rgba(255,238,190,.58)),
        radial-gradient(circle at 12% 0%,rgba(255,255,255,.66),transparent 38%)!important;
      border:1px solid rgba(76,178,224,.36)!important;
      box-shadow:0 -10px 32px rgba(70,140,180,.22),inset 0 1px 0 rgba(255,255,255,.70)!important;
      backdrop-filter:blur(22px) saturate(1.25);
    }

    [data-rc-theme="day"] .nav-tab-pro{
      color:#173049!important;
      background:rgba(255,255,255,.23)!important;
      border:1px solid rgba(255,255,255,.20)!important;
    }

    [data-rc-theme="day"] .nav-tab-pro[data-active="true"]{
      color:#07131c!important;
      background:
        linear-gradient(180deg,rgba(255,249,216,.92),rgba(162,227,255,.76))!important;
      box-shadow:inset 0 0 0 1px rgba(255,255,255,.72),0 8px 20px rgba(34,130,180,.22)!important;
    }

    [data-rc-theme="day"] .nav-icon-pro{
      color:#07131c!important;
      background:rgba(255,255,255,.42)!important;
      border:1px solid rgba(64,170,220,.24)!important;
    }

    [data-rc-theme="day"] .nav-tab-pro span{
      color:#10253a!important;
      font-weight:950!important;
    }

    [data-rc-theme="day"] .premium-home section,
    [data-rc-theme="day"] .rc-visual-rework .rc-hero-wall,
    [data-rc-theme="day"] .rc-art-card{
      color:#fff7df!important;
      background:
        linear-gradient(120deg,rgba(6,20,30,.93),rgba(19,65,88,.82) 46%,rgba(62,37,12,.66)),
        radial-gradient(circle at 20% 20%,rgba(96,225,255,.28),transparent 30%),
        radial-gradient(circle at 86% 24%,rgba(255,213,100,.22),transparent 28%)!important;
      border-color:rgba(203,242,255,.30)!important;
      box-shadow:0 24px 58px rgba(22,56,78,.26),inset 0 1px 0 rgba(255,255,255,.14)!important;
    }
    ...
    [data-rc-theme="night"].app-shell,
    [data-rc-theme="night"].rc-standalone-shell{
      color:var(--rc-night-text)!important;
      background:
        radial-gradient(circle at 14% 4%,rgba(70,200,255,.20),transparent 28%),
        radial-gradient(circle at 88% 10%,rgba(240,200,92,.16),transparent 28%),
        radial-gradient(circle at 82% 88%,rgba(177,76,255,.18),transparent 30%),
        radial-gradient(circle at 18% 84%,rgba(217,74,53,.13),transparent 28%),
        linear-gradient(180deg,#08080d 0%,#0b1020 46%,#140b11 100%)!important;
      box-shadow:0 0 0 1px rgba(240,200,92,.13),0 0 68px rgba(83,112,255,.14)!important;
    }

    [data-rc-theme="night"] .page-content-pro{
      color:var(--rc-night-text)!important;
      background:
        linear-gradient(180deg,rgba(10,13,24,.76),rgba(13,9,14,.82)),
        radial-gradient(circle at 20% 0%,rgba(54,224,188,.12),transparent 25%),
        radial-gradient(circle at 92% 12%,rgba(177,76,255,.13),transparent 28%),
        radial-gradient(circle at 70% 94%,rgba(240,200,92,.10),transparent 30%)!important;
      border:1px solid rgba(255,231,164,.16)!important;
      box-shadow:inset 0 1px 0 rgba(255,255,255,.07),0 18px 60px rgba(0,0,0,.34)!important;
    }

    [data-rc-theme="night"] .app-header-pro,
    [data-rc-theme="night"] .rc-balanced-topbar{
      background:
        linear-gradient(135deg,rgba(8,10,18,.94),rgba(21,17,44,.84) 50%,rgba(48,26,14,.72)),
        radial-gradient(circle at 16% 0%,rgba(54,224,188,.22),transparent 36%)!important;
      border:1px solid rgba(255,231,164,.18)!important;
      box-shadow:0 16px 38px rgba(0,0,0,.34),inset 0 1px 0 rgba(255,255,255,.10)!important;
    }
    ...
    [data-rc-theme="night"] .modal-panel-pro{
      color:var(--rc-night-text)!important;
      background:
        linear-gradient(155deg,rgba(13,16,30,.92),rgba(18,12,25,.88)),
        radial-gradient(circle at 18% 0%,rgba(54,224,188,.10),transparent 34%)!important;
      border:1px solid rgba(255,231,164,.17)!important;
      box-shadow:0 16px 34px rgba(0,0,0,.34),inset 0 1px 0 rgba(255,255,255,.08)!important;
    }

    [data-rc-theme="night"] .page-content-pro > div > .card:nth-child(3n+1),
    [data-rc-theme="night"] .page-content-pro .studio-panel:nth-child(3n+1){
      background:
        linear-gradient(155deg,rgba(13,18,34,.94),rgba(9,25,28,.84)),
        radial-gradient(circle at 88% 10%,rgba(54,224,188,.13),transparent 32%)!important;
    }

    [data-rc-theme="night"] .page-content-pro > div > .card:nth-child(3n+2),
    [data-rc-theme="night"] .page-content-pro .studio-panel:nth-child(3n+2){
      background:
        linear-gradient(155deg,rgba(19,13,34,.94),rgba(34,16,40,.82)),
        radial-gradient(circle at 85% 10%,rgba(177,76,255,.14),transparent 32%)!important;
    }

    [data-rc-theme="night"] .page-content-pro > div > .card:nth-child(3n),
    [data-rc-theme="night"] .page-content-pro .studio-panel:nth-child(3n){
      background:
        linear-gradient(155deg,rgba(26,16,13,.94),rgba(24,21,11,.82)),
        radial-gradient(circle at 85% 10%,rgba(240,200,92,.13),transparent 32%)!important;
    }

    [data-rc-theme="night"] input,
    [data-rc-theme="night"] textarea,
    [data-rc-theme="night"] select{
      background:rgba(7,10,18,.90)!important;
      color:#fff7df!important;
      border:1.5px solid rgba(100,210,255,.25)!important;
      box-shadow:inset 0 1px 0 rgba(255,255,255,.08),0 4px 12px rgba(0,0,0,.18)!important;
    }

    [data-rc-theme="night"] input::placeholder,
    [data-rc-theme="night"] textarea::placeholder{
      color:rgba(255,247,218,.54)!important;
    }

    [data-rc-theme="night"] .bottom-nav-pro{
      background:
        linear-gradient(135deg,rgba(8,10,18,.92),rgba(20,18,43,.86) 52%,rgba(24,17,10,.86)),
        radial-gradient(circle at 16% 0%,rgba(54,224,188,.16),transparent 38%)!important;
      border:1px solid rgba(100,210,255,.18)!important;
      box-shadow:0 -8px 34px rgba(0,0,0,.42),inset 0 1px 0 rgba(255,255,255,.08)!important;
      backdrop-filter:blur(20px) saturate(1.16);
    }

    [data-rc-theme="night"] .nav-tab-pro[data-active="true"]{
      background:
        linear-gradient(180deg,rgba(61,225,198,.17),rgba(240,200,92,.13))!important;
      box-shadow:inset 0 0 0 1px rgba(255,244,214,.12),0 6px 20px rgba(54,224,188,.10)!important;
    }

    [data-rc-theme="night"] .nav-tab-pro span{
      color:rgba(255,247,218,.88)!important;
      font-weight:950!important;
    }

    [data-rc-theme="night"] .premium-home section,
    [data-rc-theme="night"] .rc-visual-rework .rc-hero-wall{
      background:
        linear-gradient(120deg,rgba(5,7,13,.96),rgba(15,23,45,.88) 42%,rgba(52,24,13,.70)),
        radial-gradient(circle at 20% 20%,rgba(54,224,188,.22),transparent 28%),
        radial-gradient(circle at 78% 24%,rgba(177,76,255,.20),transparent 28%),
        radial-gradient(circle at 84% 84%,rgba(240,200,92,.14),transparent 28%)!important;
      border-color:rgba(255,231,164,.20)!important;
    }

    @media(max-width:560px){
      [data-rc-theme="day"] .page-content-pro{
        background:
          linear-gradient(180deg,rgba(255,250,232,.82),rgba(238,222,188,.76)),
          radial-gradient(circle at 18% 0%,rgba(110,216,255,.18),transparent 32%)!important;
      }
      [data-rc-theme="day"] .bottom-nav-pro{
        background:linear-gradient(135deg,rgba(220,247,255,.86),rgba(188,232,255,.72) 54%,rgba(255,238,190,.72))!important;
      }
      [data-rc-theme="night"] .page-content-pro{
        background:
          linear-gradient(180deg,rgba(10,13,24,.84),rgba(13,9,14,.88)),
          radial-gradient(circle at 18% 0%,rgba(54,224,188,.10),transparent 30%)!important;
      }
    }
