    ...
    <div style={{position:"fixed",inset:0,background:"rgba(27,67,50,0.55)",zIndex:500,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:T.white,borderRadius:"22px 22px 0 0",padding:"24px 18px 32px",width:"100%",maxWidth:480,animation:"slideUp 0.3s ease",maxHeight:"90vh",overflowY:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          ...
          <button onClick={onClose} style={{background:T.g150,border:"none",borderRadius:"50%",width:32,height:32,cursor:"pointer",fontSize:"1rem",color:T.g700}}>✕</button>
        </div>
        {children}
      </div>
    </div>
...
function PtsPopup({pts,show}){
  ...
  return(
    <div style={{position:"fixed",top:"35%",left:"50%",transform:"translateX(-50%)",zIndex:9999,animation:"ptsFloat 1.8s ease forwards",pointerEvents:"none"}}>
      <div style={{background:T.gradGold,color:T.white,borderRadius:50,padding:"10px 24px",fontWeight:900,fontSize:"1.4rem",boxShadow:"0 6px 24px rgba(255,183,3,0.5)"}}>+{pts} ⭐</div>
    </div>
  );
}
function Particles(){
...
function Auth({onLogin,showToast}){
  ...
  function normalizeUser(raw){
    return{
      ...
      email:raw[U.email]||raw["correo electrónico"]||raw.email||"",
  ...
  async function handleLogin(){
    ...
    setLoading(false);
    if(!users||users.length===0){showToast("❌ Email o contraseña incorrectos");SFX.error();return;}
  ...
  async function handleRegister(){
    ...
    if(existing&&existing.length>0){setLoading(false);showToast("❌ Ese email ya está registrado");SFX.error();return;}
    ...
    setLoading(false);
    if(!newUser||newUser.length===0){showToast("❌ Error al registrar");SFX.error();return;}
    SFX.success();showToast("✅ ¡Bienvenid@ a PeluquerIA!");onLogin(normalizeUser(newUser[0]));
  }
    ...
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,${T.g800} 0%,${T.g600} 60%,${T.g400} 100%)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px"}}>
      ...
      <div style={{position:"relative",zIndex:1,width:"100%",maxWidth:400}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{fontSize:"3.5rem",animation:"bounce 2s ease infinite"}}>✂️</div>
          <div style={{fontFamily:"'Fredoka One',cursive",fontSize:"2.4rem",color:T.white}}>PeluquerIA</div>
          <div style={{color:T.g200,fontSize:"0.85rem",marginTop:4,fontWeight:600}}>Tu peluquería inteligente ✨</div>
        </div>
...
function DashboardAdmin({user}){
  ...
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
      ...
      <Card>
        <div style={{fontWeight:800,fontSize:"0.95rem",color:T.g800,marginBottom:12}}>📅 Próximas citas</div>
        {citasHoy.length===0?<EmptyState icon="✂️" title="Sin citas hoy" sub="¡Día tranquilo!"/>
          :citasHoy.map(c=>(
    ...
    <div style={{animation:"fadeSlide 0.4s ease"}}>
      <Card style={{background:T.gradClient,border:"none",marginBottom:16,padding:"20px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div style={{textAlign:"center"}}>
            ...
            <div style={{fontSize:"0.7rem",color:"rgba(255,255,255,0.75)"}}>⭐</div>
          </div>
        </div>
      ...
      {proxCita&&(
        <Card style={{marginBottom:16,background:T.g50}}>
          <div style={{fontWeight:800,color:T.g700,marginBottom:8}}>📅 Tu próxima cita</div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
      ...
      {noticias.length>0&&(
        <div>
          ...
          {noticias.map(n=>(
            <Card key={n.identificación||n.id} style={{marginBottom:10}} hover>
...
// CITAS
const SERVICIOS=[
  {id:"corte",label:"✂️ Corte",precio:15},{id:"color",label:"🎨 Coloración",precio:45},
...
function Citas({user,showToast}){
  ...
  async function saveCita(){
    ...
    showToast("✅ Cita reservada");SFX.success();setShowNew(false);
  ...
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
      ...
      {loading?<Spinner/>:citas.length===0?<EmptyState icon="📅" title="Sin citas" sub="¡Reserva la primera!"/>
        :citas.map(c=>(
          <Card key={c.id||c.identificación} style={{marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div>
                ...
                <div style={{fontSize:"0.78rem",color:T.textSub,marginTop:2}}>{c.fecha&&new Date(c.fecha).toLocaleDateString("es-ES",{day:"numeric",month:"short"})} · {c.hora}</div>
              </div>
            ...
            {c.estado==="pendiente"&&(
              <div style={{marginTop:10,display:"flex",gap:8}}>
                {isAdmin&&<Btn small col="green" onClick={()=>{dbPatch(TB.citas,`?id=eq.${c.id}`,{estado:"confirmada"});loadCitas();}}>✅ Confirmar</Btn>}
                <Btn small col="red" onClick={()=>{dbPatch(TB.citas,`?id=eq.${c.id}`,{estado:"cancelada"});loadCitas();}}>❌ Cancelar</Btn>
              </div>
            )}
          </Card>
        ))
...
function Clientes({showToast}){
  ...
  const filtered=clientes.filter(c=>(c[U.nombre]||"").toLowerCase().includes(search.toLowerCase())||(c[U.email]||c["correo electrónico"]||"").toLowerCase().includes(search.toLowerCase()));
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
        ...
        <Card key={c[U.id]||c.id} style={{marginBottom:10}} hover onClick={()=>selectCliente(c)}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:"0.78rem",color:T.textSub}}>{c[U.email]||c["correo electrónico"]}</div>
            </div>
            <div style={{fontWeight:900,color:T.g600}}>⭐ {c[U.puntos]||c.puntos||0}</div>
          </div>
        </Card>
      ))}
      <Modal show={!!selected} onClose={()=>setSelected(null)} title={`👤 ${selected?.[U.nombre]||selected?.nombre||""}`}>
        {selected&&(
          <div>
            <div style={{display:"flex",gap:12,marginBottom:16,alignItems:"center"}}>
              ...
              <div>
                ...
                <div style={{fontSize:"0.82rem",color:T.textSub}}>{selected[U.email]||selected["correo electrónico"]}</div>
              </div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
              <StatCard icon="⭐" label="Puntos" value={selected[U.puntos]||selected.puntos||0} col="gold"/>
            ...
            {historial.length===0?<EmptyState icon="📅" title="Sin citas" sub=""/>:historial.map(h=>(
...
function Inventario({showToast}){
  ...
  async function saveItem(){if(!form.nombre){showToast("Escribe un nombre");return;}await dbPost(TB.inventario,form);showToast("✅ Añadido");setShowNew(false);setForm({nombre:"",categoria:"coloracion",stock:0,stock_min:5,precio_compra:0,precio_venta:0});load();}
  ...
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
      ...
      {items.filter(i=>i.stock<=i.stock_min).length>0&&<Card style={{background:"#FFEBEE",border:`1px solid ${T.red}`,marginBottom:14}}><div style={{fontWeight:800,color:T.red,fontSize:"0.88rem"}}>⚠️ {items.filter(i=>i.stock<=i.stock_min).length} productos con stock bajo</div></Card>}
      ...
      <Modal show={showNew} onClose={()=>setShowNew(false)} title="📦 Nuevo producto">
        ...
        <Select label="Categoría" value={form.categoria} onChange={v=>setForm(f=>({...f,categoria:v}))} options={CATS.map(c=>({value:c,label:c.charAt(0).toUpperCase()+c.slice(1)}))}/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <Input label="Stock" value={form.stock} onChange={v=>setForm(f=>({...f,stock:+v}))} type="number"/>
          <Input label="Mínimo" value={form.stock_min} onChange={v=>setForm(f=>({...f,stock_min:+v}))} type="number"/>
        ...
        :ventas.map(v=>(
          <Card key={v.id} style={{marginBottom:8}}>
            <div style={{display:"flex",justifyContent:"space-between"}}>
              <div>
                <div style={{fontWeight:800}}>{v.cliente_nombre||"Anónimo"}</div>
        ...
        {carrito.length>0&&(
          <div style={{background:T.g50,borderRadius:12,padding:12,marginBottom:14}}>
            ...
            <div style={{borderTop:`1px solid ${T.g200}`,marginTop:8,paddingTop:8,fontWeight:900,display:"flex",justifyContent:"space-between"}}>
              <span>TOTAL</span><span style={{color:T.g600}}>{total.toFixed(2)}€</span>
            </div>
          </div>
        )}
        <Select label="Método de pago" value={metodo} onChange={setMetodo} options={[{value:"efectivo",label:"💶 Efectivo"},{value:"tarjeta",label:"💳 Tarjeta"},{value:"bizum",label:"📱 Bizum"}]}/>
...
function AdminUsuarios({showToast}){
  ...
  async function changeRole(id,rol){await dbPatch(TB.usuarios,`?${encodeURIComponent(U.id)}=eq.${id}`,{[U.rol]:rol});showToast("✅ Rol actualizado");load();}
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
        ...
        <Card key={u[U.id]||u.id} style={{marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:"0.75rem",color:T.textSub}}>{u[U.email]||u["correo electrónico"]}</div>
            </div>
...
// FEED — usa tu tabla publicaciones
function SocialFeed({user,setUser,showToast,showPoints}){
  ...
  async function publish(){
    ...
    await dbPost(TB.publicaciones,{contenido:newPost,autor_id:user.id,titulo:"",tipo:"post",emoji:"💬","número_de_me_g":0});
  ...
  async function likePost(post){
    const id=post.identificación||post.id;
    const likes=(post["número_de_me_g"]||0)+1;
    await dbPatch(TB.publicaciones,`?identificación=eq.${id}`,{"número_de_me_g":likes});load();
  }
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
      <SectionHeader icon="📱" title="Feed" sub="Comparte con la comunidad"/>
      <Card style={{marginBottom:16}}>
        <textarea value={newPost} onChange={e=>setNewPost(e.target.value)} placeholder="¿Qué te parece tu nuevo look? ✨" rows={3}
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:8}}>
          <span style={{fontSize:"0.75rem",color:T.textSub}}>+5⭐ por publicar</span>
          <Btn small onClick={publish}>📤 Publicar</Btn>
        </div>
      </Card>
      {loading?<Spinner/>:posts.map(p=>(
        <Card key={p.identificación||p.id} style={{marginBottom:12}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:8}}>
            ...
            <button onClick={()=>likePost(p)} style={{background:"none",border:"none",cursor:"pointer",fontSize:"0.82rem",color:T.textSub,fontWeight:700}}>❤️ {p["número_de_me_g"]||0}</button>
          </div>
        </Card>
      ))}
    </div>
...
function Tienda({user,setUser,showToast,showPoints}){
  ...
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
      <SectionHeader icon="🛍️" title="Tienda" sub={`Tienes ${user.puntos||0} ⭐`}/>
      <Card style={{background:T.gradGold,border:"none",marginBottom:16,padding:"14px 16px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{color:T.white}}>
            ...
            <div style={{fontFamily:"'Fredoka One',cursive",fontSize:"2rem"}}>{user.puntos||0} ⭐</div>
          </div>
          <div style={{fontSize:"2.5rem"}}>🎁</div>
        </div>
      </Card>
      {loading?<Spinner/>:productos.length===0?<EmptyState icon="🛍️" title="Sin premios aún" sub="¡Pronto habrá novedades!"/>
        :productos.map(p=>{
          ...
          return(
            <Card key={p.id} style={{marginBottom:12,border:canClaim?`2px solid ${T.g400}`:`1px solid ${T.g150}`,opacity:canClaim?1:0.75}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                ...
                <div style={{fontWeight:900,color:T.orange,fontSize:"1.1rem",marginLeft:12}}>{p.puntos_precio} ⭐</div>
              </div>
              <div style={{marginTop:12}}>
                {canClaim?<Btn full small col="gold" onClick={()=>canjear(p)}>🎁 Canjear</Btn>
                  :<div style={{textAlign:"center",fontSize:"0.78rem",color:T.textSub,fontWeight:700}}>Te faltan {p.puntos_precio-(user.puntos||0)} ⭐</div>}
              </div>
            </Card>
          );
        })
...
function Cupones({user,showToast}){
  ...
  async function validar(){
    ...
    if(!found){showToast("❌ Cupón no válido");SFX.error();return;}
    if(new Date(found.fecha_fin)<new Date()){showToast("❌ Cupón caducado");SFX.error();return;}
    SFX.coins();showToast(`✅ ${found.descuento}% de descuento — ¡válido!`);
  }
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
      <SectionHeader icon="🏷️" title="Cupones" sub="Descuentos exclusivos"/>
      <Card style={{marginBottom:16}}>
        <div style={{fontWeight:800,color:T.g700,marginBottom:10}}>🔍 Validar cupón</div>
  ...
  return(
    <div>
      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
        {SOPA_WORDS.map(w=><Badge key={w} col={found.includes(w)?"green":"gold"}>{found.includes(w)?"✅ ":""}{w}</Badge>)}
      </div>
...
const TRIVIA_QS=[
  {q:"¿Cuántos volúmenes tiene el tinte permanente más común?",opts:["20 vol","30 vol","10 vol","40 vol"],a:1},
  {q:"¿Qué vitamina es esencial para el cabello sano?",opts:["Vitamina C","Vitamina K","Biotina (B7)","Vitamina D"],a:2},
  {q:"¿Cuál es el pH ideal del cabello?",opts:["4.5–5.5","7–8","2–3","6–7"],a:0},
  {q:"¿Qué es la queratina?",opts:["Un tinte","Una proteína capilar","Un champú","Una vitamina"],a:1},
  {q:"¿Cada cuánto se recomienda cortar las puntas?",opts:["Cada año","Cada 6-8 semanas","Cada semana","Cada 6 meses"],a:1},
];
    ...
    <div>
      ...
      {q.opts.map((o,i)=>(
        <button key={i} onClick={()=>answer(i)} style={{display:"block",width:"100%",textAlign:"left",padding:"10px 14px",borderRadius:12,marginBottom:8,border:"2px solid",
          ...
          {answered!==null&&i===q.a?"✅ ":answered===i&&i!==q.a?"❌ ":""}{o}
        </button>
      ))}
    </div>
  );
...
function Juegos({user,setUser,showToast,showPoints}){
  ...
  const GAMES=[
    {id:"sopa",icon:"🔤",title:"Sopa de Letras",desc:"Encuentra las palabras ocultas",pts:25},
    {id:"memoria",icon:"🧠",title:"Memoria",desc:"Encuentra todos los pares",pts:20},
    {id:"trivia",icon:"❓",title:"Trivia Capilar",desc:"Demuestra tu conocimiento",pts:15},
  ];
  ...
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
      <SectionHeader icon="🎮" title="Juegos" sub="Gana puntos — 1 vez al día por juego"/>
      {GAMES.map(g=>{
          ...
          <Card key={g.id} style={{marginBottom:12,opacity:played?0.65:1}}>
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:"0.75rem",color:T.orange,fontWeight:700,marginTop:2}}>Hasta +{g.pts} ⭐</div>
              </div>
...
// RETOS — usa tus tablas retos y retos_progreso con identificación uuid
function Retos({user,setUser,showToast,showPoints}){
  ...
  async function reclamar(reto){
    const id=reto.identificación||reto.id;
    const prog=progresos[id];if(!prog||prog.completado)return;
    const progId=prog.identificación||prog.id;
    await dbPatch(TB.retos_progreso,`?identificación=eq.${progId}`,{completado:true});
  ...
  function daysLeft(f){const d=Math.ceil((new Date(f)-new Date())/86400000);return d<=0?"Vence hoy":`${d} días`;}
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
      ...
      {loading?<Spinner/>:retos.length===0?<EmptyState icon="🎯" title="Sin retos activos" sub="¡Vuelve pronto!"/>
        :retos.map(r=>{
          const id=r.identificación||r.id;
            ...
            <Card key={id} style={{marginBottom:12,border:canClaim?`2px solid ${T.g400}`:done?`2px solid ${T.g300}`:`1px solid ${T.g150}`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:"0.8rem",color:T.textSub,marginTop:2}}>{r.descripción||r.descripcion}</div>
                </div>
                <div style={{textAlign:"right",marginLeft:10}}>
                  <div style={{fontWeight:900,color:T.pink,fontSize:"1rem"}}>+{r.puntos_premio} ⭐</div>
              ...
              {done?<div style={{background:T.g100,borderRadius:10,padding:"8px 12px",fontSize:"0.82rem",fontWeight:700,color:T.g700}}>✅ Reto completado</div>
                ...
                :<div style={{fontSize:"0.78rem",color:T.textSub,fontWeight:600}}>{r.meta-pv} más para completarlo</div>}
            </Card>
          );
        })
      }
        ...
        return(
          <Card key={uid} style={{marginBottom:8,background:isMe?T.g100:T.white,border:isMe?`2px solid ${T.g400}`:`1px solid ${T.g150}`}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              ...
              <div style={{flex:1}}><div style={{fontWeight:800}}>{u[U.nombre]||u.nombre}{isMe?" (tú)":""}</div></div>
              <div style={{fontWeight:900,color:T.orange}}>⭐ {u[U.puntos]||u.puntos||0}</div>
            </div>
          </Card>
        );
      })}
...
function Galeria({showToast,isAdmin=false}){
  ...
  async function save(){if(!form.url){showToast("Añade una URL");return;}await dbPost(TB.galeria,{...form,activo:true});showToast("✅ Añadido");setShowNew(false);setForm({titulo:"",url:"",categoria:"corte",antes_url:""});load();}
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
      <SectionHeader icon="🖼️" title="Galería" sub="Nuestros mejores trabajos" action={isAdmin&&<Btn small onClick={()=>setShowNew(true)}>+ Añadir</Btn>}/>
      {fotos.length===0?<EmptyState icon="🖼️" title="Sin fotos aún" sub="¡Añade los primeros trabajos!"/>:(
      ...
      <Modal show={showNew} onClose={()=>setShowNew(false)} title="🖼️ Añadir trabajo">
        <Input label="URL imagen (después)" value={form.url} onChange={v=>setForm(f=>({...f,url:v}))} placeholder="https://..."/>
        <Input label="URL imagen antes (opcional)" value={form.antes_url} onChange={v=>setForm(f=>({...f,antes_url:v}))} placeholder="https://..."/>
        <Input label="Título" value={form.titulo} onChange={v=>setForm(f=>({...f,titulo:v}))}/>
        <Select label="Categoría" value={form.categoria} onChange={v=>setForm(f=>({...f,categoria:v}))} options={CATS.map(c=>({value:c,label:c.charAt(0).toUpperCase()+c.slice(1)}))}/>
...
function Reviews({user,setUser,showToast,showPoints}){
  ...
  async function submit(){
    ...
    await dbPatch(TB.usuarios,`?${encodeURIComponent(U.id)}=eq.${user.id}`,{[U.puntos]:nuevos});
    setUser(u=>({...u,puntos:nuevos}));showPoints(10);showToast("✅ ¡Gracias! +10⭐");
  ...
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
      <SectionHeader icon="⭐" title="Reseñas" sub={`${avg}★ · ${reviews.length} valoraciones`} action={user.rol===ROLES.CLIENT&&<Btn small onClick={()=>setShowNew(true)}>+ Reseña</Btn>}/>
      {loading?<Spinner/>:reviews.map(r=>(
        <Card key={r.id} style={{marginBottom:10}}>
          <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:8}}>
            ...
            <div style={{fontWeight:900,color:T.gold}}>{Array(r.rating).fill("★").join("")}</div>
          </div>
          <div style={{fontSize:"0.88rem",color:T.text}}>{r.comentario}</div>
        </Card>
      ))}
      <Modal show={showNew} onClose={()=>setShowNew(false)} title="⭐ Nueva reseña">
        <div style={{marginBottom:16}}>
          <div style={{fontSize:"0.8rem",fontWeight:800,color:T.g700,marginBottom:8}}>Tu puntuación</div>
        ...
        <Input label="Comentario" value={comment} onChange={setComment} placeholder="Cuéntanos tu experiencia..."/>
        <Btn full onClick={submit}>✅ Enviar (+10⭐)</Btn>
      </Modal>
    </div>
  );
}
...
function Perfil({user,setUser,onLogout,showToast}){
  ...
  const nivel=user.puntos>=1000?{label:"💎 VIP",col:T.blue}:user.puntos>=500?{label:"🥇 Gold",col:T.gold}:user.puntos>=200?{label:"🥈 Silver",col:"#9E9E9E"}:{label:"🥉 Bronze",col:T.orange};
  return(
    <div style={{animation:"fadeSlide 0.4s ease"}}>
      ...
      <Card style={{textAlign:"center",marginBottom:16}}>
        ...
        {user.rol===ROLES.CLIENT&&<div style={{marginTop:12}}><div style={{fontFamily:"'Fredoka One',cursive",fontSize:"1.8rem",color:T.g700}}>⭐ {user.puntos||0}</div><div style={{fontSize:"0.75rem",color:T.textSub}}>puntos acumulados</div></div>}
      </Card>
      {editing?(
        <Card style={{marginBottom:16}}>
          <Input label="Nombre" value={form.nombre} onChange={v=>setForm(f=>({...f,nombre:v}))}/>
      ...
      ):<Btn full col="ghost" onClick={()=>setEditing(true)} style={{marginBottom:12}}>✏️ Editar perfil</Btn>}
  ...
  return(
    <div style={{fontFamily:"'Nunito',sans-serif",background:T.g100,minHeight:"100vh",maxWidth:480,margin:"0 auto",paddingBottom:82,position:"relative"}}>
      ...
      <div style={{background:grad,padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:50,boxShadow:"0 4px 20px rgba(27,67,50,0.25)"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{fontFamily:"'Fredoka One',cursive",fontSize:"1.35rem",color:T.white}}>✂️ PeluquerIA</div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <button onClick={toggleMusic} style={{background:"rgba(255,255,255,0.18)",border:"none",borderRadius:50,padding:"5px 10px",cursor:"pointer",color:T.white,fontWeight:800,fontSize:"0.72rem"}}>{musicOn?"🔇":"🎵"}</button>
          {role===ROLES.CLIENT&&<div style={{background:"rgba(255,255,255,0.2)",borderRadius:50,padding:"4px 12px",color:T.white,fontWeight:900,fontSize:"0.84rem"}}>⭐ {user.puntos||0}</div>}
