...
function resolveFreq(value){
  return typeof value==="number"?value:(NOTE_FREQ[value]||PENTA[0]);
}
...
function playNoise(dur=0.05,vol=0.02,delay=0,type="highpass",freq=1800){
  ...
  try{
    ...
    src.buffer=buffer;
    filter.type="lowpass";
    filter.frequency.value=Math.min(freq,1400);
    filter.Q.value=.42;
    ...
    src.stop(ctx.currentTime+delay+dur+0.12);
  }catch(e){}
}
...
function playRastaVoice(kind="talk"){
  ...
  try{
    const voices={
      open:[[523,.050,.012,0],[659,.055,.014,.055],[784,.070,.010,.115],[1046,.050,.006,.190]],
      close:[[659,.045,.010,0],[523,.052,.009,.055],[392,.070,.007,.115]],
      tip:[[587,.050,.012,0],[740,.060,.012,.060],[880,.045,.010,.135],[740,.050,.006,.205]],
      help:[[392,.050,.010,0],[523,.055,.012,.050],[392,.045,.008,.115],[659,.070,.010,.175]],
      context:[[440,.040,.010,0],[660,.052,.012,.052],[880,.054,.010,.116]],
      happy:[[523,.045,.012,0],[659,.045,.012,.045],[784,.055,.013,.095],[988,.070,.009,.155]]
    };
    ...
    if(kind==="open"||kind==="tip"||kind==="happy"){
      playNoise(.035,.004,.075,"lowpass",1250);
    }
  }catch(e){}
}
...
function playUiSound(kind="tap"){
  ...
  (patterns[kind]||patterns.tap).forEach(([f,d,v,delay])=>{
    playTone(f,"sine",d,v,delay);
  });
}
...
function beatHit(list,beat){
  return Array.isArray(list)&&list.includes(beat);
}
...
function tickLofiTrack(){
  ...
  try{
    ...
    if(beatHit(g.skank,beat)){
      ...
      if((bar+beat)%4===0){
        playChord(chord,"piano",.18,.006,0.24+swing);
      }
    }

    if(beatHit(g.ghost,beat) && bar%2===0){
      playChord(chord,"piano",.14,.005,0.04+swing);
    }
    ...
    if(melodyNote && beatHit(g.melody,beat)){
      ...
      if(nextNote && (beat===3||beat===11) && bar%2===0){
        playInstrument(nextNote,leadKind,.28,.009,0.24+swing);
      }
    }

    if(counterNote && beatHit(g.counter,beat)){
      playInstrument(counterNote,"piano",.34,.009,0.10+swing);
    }

    if(g.padEvery && step%g.padEvery===0){
      chord.forEach((n,i)=>{
        playInstrument(n,"violin",1.45,.005,0.08+i*.055);
      });
    }

    if(Number.isFinite(g.arp) && step%64===g.arp){
      [...chord].reverse().forEach((n,i)=>{
        playInstrument(n,"pan",.22,.007,0.06+i*.08);
      });
    }
...
function normalizeAudioZone(zone){
  ...
  if(["home","login","shop","profile","arcade","community","global"].includes(z)){
    return z;
  }
...
function backgroundCandidateIndexes(){
  ...
  if(backgroundMusicMode!=="ambient"){
    return Array.from({length:len},(_,i)=>i);
  }

  const zone=normalizeAudioZone(backgroundPreferredZone);
  ...
  return global.length?global:Array.from({length:len},(_,i)=>i);
}
...
function startGeneratedMusic(){
  ...
  if(!globalMuted&&!backgroundDuckedForGame){
    tickLofiTrack();
  }
}
...
function muteMusicKeepTime(muted=true){
  ...
  if(musicPlaying&&!backgroundAudioAvailable&&!globalMuted&&!backgroundDuckedForGame){
    startGeneratedMusic();
  }
}
...
function setBackgroundMusicMode(mode){
  ...
  try{
    localStorage.setItem("rasta_cuts_audio_mode",backgroundMusicMode);
  }catch{}
}
